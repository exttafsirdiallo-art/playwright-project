...
test.describe("TC23: Verify address details in checkout page @cart_checkout", () => {
  test("Test Case 23: Verify address details in checkout page JDD faker et JSON", async ({
    ...
    //Attendre que le bloc adresse de facturation soit visible
    const invoice = page.locator("#address_invoice");
    await expect(invoice).toBeVisible({ timeout: 10000 });

    //Petite fonction pour normaliser les espaces et retours à la ligne
    const normalizee = (s: string) => s.replace(/\s+/g, " ").trim();

    // 1️⃣ Nom (vérifie prénom + nom séparément pour tolérer ponctuation/espaces)
    const nameEle = invoice.locator(".address_firstname.address_lastname");
    await expect(nameEle).toContainText(prenom);
    await expect(nameEle).toContainText(nom);

    // 2️⃣ Rue / adresse
    await expect(invoice).toContainText(address);

    // 3️⃣ Ville / État / Code postal
    const cityStateZipEle = invoice.locator(
      ".address_city.address_state_name.address_postcode"
    );
    const cityStateZipe = normalizee(await cityStateZipEle.innerText());
    expect(cityStateZip).toContain(normalize(city));
    expect(cityStateZip).toContain(normalize(state));
    expect(cityStateZip).toContain(normalize(zipCode));

    // 4️⃣ Pays
    await expect(invoice.locator(".address_country_name")).toHaveText(
      u3.country
    );

    // 5️⃣ Téléphone
    await expect(invoice.locator(".address_phone")).toContainText(phone);

    console.log(
      "✅ Les informations de facturation correspondent bien aux données Faker."
    );
