...
test.describe("TC24: Download Invoice after purchase order @cart_checkout @smoke", () => {
  test("Test Case 24: Download Invoice after purchase order JDD faker", async ({
    ...
    // 2) Attendre le modal puis cliquer sur "Register / Login"
    ...
    // 3) Cette fois on attend la navigation vers /login
    await Promise.all([
    ...
    // Aller sur le panier
    await page.waitForSelector('a[href="/view_cart"]', {
      state: "visible",
      timeout: 10000,
    });
    await page.click('a[href="/view_cart"]');

    //Cliquer sur le checkout
    ...
    // Cliquer sur Download invoice

    // Attendre l'événement de téléchargement au moment du clic
    const [download] = await Promise.all([
      page.waitForEvent("download"),
      page.getByRole("link", { name: /download invoice/i }).click(),
    ]);

    // Utiliser le nom proposé par le navigateur + ajout d’un timestamp
    const suggested = await download.suggestedFilename();
    const timestamp = Date.now();
    const filePath = `tests/downloads/${timestamp}-${suggested}`;

    await download.saveAs(filePath);

    console.log(`✅ Fichier téléchargé : ${filePath}`);

    //Cliquer sur le bouton "Continue" après le téléchargement
    await page.getByRole("link", { name: /^continue$/i }).click();

    //Vérifier qu’on est bien revenu sur la page d’accueil
